...
@mcp.tool()
def get_metric_data(run_hashes: list[str], metrics: list[dict], n: int = 50) -> str:
    """Get sampled metric data for multiple runs. metrics is a list of {"name": str, "context": dict | None}."""
    ...
    try:
        ...
        for run_hash in run_hashes:
            r = repo.get_run(run_hash)
            if not r:
                out[run_hash] = {"error": "run not found"}
                continue
